...
class Accused(Base):
    ...
    aliases: Mapped[dict | None] = mapped_column(JSONB, nullable=True)  # ["alias1", "alias2"]
    ...
    profile_scores: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    ...
    incident_timeline: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    # Expected: [{"year": "2019", "incidents": 1}, ...]

    # Known associates (accused_ids)
    associate_ids: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
